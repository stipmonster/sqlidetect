...
function testForSqli(url) {
	...
	if (/ASC/.test(url) || /DESC/.test(url)) {
		var total=url.match(/ASC/g).length + url.match(/DESC/g).length;
		score=score+(2*total);
	}
	if (/AND/.test(url) || /OR/.test(url)) {
		var total=url.match(/AND/g).length + url.match(/OR/g).length;
		score=score+(1*total);
	}
	if (/LIKE/.test(url)) {
	...
	if (/SELECT/.test(url) || /GROUP BY/.test(url)|| /GROUP%20BY/.test(url)) {
		var total=url.match(/SELECT/g).length + url.match(/GROUP%20BY/g).length;
		score=score+(5*total);
	}
